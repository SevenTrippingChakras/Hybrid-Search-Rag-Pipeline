...
#
#   app/rag/eval/result/<strategy>/<strategy>.json   raw scores
#   app/rag/eval/result/<strategy>/<strategy>.html   single-strategy report
#   app/rag/eval/result/final/comparison.html        combined comparison
#
# Usage:
...
# Repo root, relative to this script at app/rag/eval/scripts/.
...
RESULT_DIR="app/rag/eval/result"
...
for strategy in "${STRATEGIES[@]}"; do
  ...
  uv run python -m app.rag.eval.ingest --corpus "$CORPUS" --strategy "$strategy"
  echo "- running eval"
  uv run python -m app.rag.eval.runner --strategy "$strategy" \
    --golden "$GOLDEN" --out "$out_dir/$strategy.json" \
    ${LIMIT_ARG[@]+"${LIMIT_ARG[@]}"}
  echo "- rendering report"
  uv run python -m app.rag.eval.report "$out_dir/$strategy.json" \
...
mkdir -p "$final_dir"
uv run python -m app.rag.eval.compare "${RESULT_FILES[@]}" \
